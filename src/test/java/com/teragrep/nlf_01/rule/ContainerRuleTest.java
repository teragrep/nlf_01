...
public final class ContainerRuleTest {
    ...
    void testIdealCase() {
        ...
        final Rule containerRule = new ContainerRule(new FakeSourceable());
        Assertions.assertTrue(containerRule.matches(parsedEvent));
        Assertions
                .assertEquals(
                        ...
                        Assertions.assertDoesNotThrow(() -> containerRule.eventType(parsedEvent).msg())
                );
    }
    ...
    void testIncorrectCase() {
        ...
        final ParsedEvent parsedEvent = testEvent(
                "src/test/resources/container_missing_keys.json", new EventPartitionContextImpl(partitionContextMap), new EventPropertiesImpl(propertiesMap), new EventSystemPropertiesImpl(systemPropertiesMap), new EnqueuedTimeImpl("2010-01-01T00:00:00"), new EventOffsetImpl("0")
        );

        final Rule containerRule = new ContainerRule(new FakeSourceable());
        Assertions.assertTrue(containerRule.matches(parsedEvent));
        Assertions.assertThrows(PluginException.class, () -> containerRule.eventType(parsedEvent).appName());
    }
}
