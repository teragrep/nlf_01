...
public final class AppInsightRuleTest {
    ...
    void testIdealCase() {
        ...
        final ParsedEvent parsedEvent = testEvent(
                "src/test/resources/appinsight.json", new EventPartitionContextImpl(partitionContextMap), new EventPropertiesImpl(propertiesMap), new EventSystemPropertiesImpl(systemPropertiesMap), new EnqueuedTimeImpl("2010-01-01T00:00:00"), new EventOffsetImpl("0")
        );

        final Rule appInsightRule = new AppInsightRule();
        Assertions.assertTrue(appInsightRule.matches(parsedEvent));
        Assertions
                .assertEquals(
                        ...
                        Assertions.assertDoesNotThrow(() -> appInsightRule.eventType(parsedEvent).msg())
                );
    }
    ...
    void testIncorrectCase() {
        ...
        final ParsedEvent parsedEvent = testEvent(
                "src/test/resources/appinsight_missing_keys.json", new EventPartitionContextImpl(partitionContextMap), new EventPropertiesImpl(propertiesMap), new EventSystemPropertiesImpl(systemPropertiesMap), new EnqueuedTimeImpl("2010-01-01T00:00:00"), new EventOffsetImpl("0")
        );

        final Rule appInsightRule = new AppInsightRule();
        Assertions.assertTrue(appInsightRule.matches(parsedEvent));
        Assertions.assertThrows(PluginException.class, () -> appInsightRule.eventType(parsedEvent).appName());
    }
}
