...
public final class SyslogTypeTest {
    ...
    void testIdealCase() {
        ...
        final ParsedEvent parsedEvent = testEvent(
                "src/test/resources/syslog.json", new EventPartitionContextImpl(fakePartitionContextMap.map()), new EventPropertiesImpl(fakePropertiesMap.map()), new EventSystemPropertiesImpl(fakeSystemPropertiesMap.map()), new EnqueuedTimeImpl("2010-01-01T00:00:00"), new EventOffsetImpl("0")
        );
