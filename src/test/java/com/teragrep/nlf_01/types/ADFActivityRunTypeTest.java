...
class ADFActivityRunTypeTest {
    ...
    void testIdealCase() {
        ...
        final ParsedEvent parsedEvent = testEvent(
                "src/test/resources/adfactivityrun.json", new EventPartitionContextImpl(fakePartitionContextMap.map()), new EventPropertiesImpl(fakePropertiesMap.map()), new EventSystemPropertiesImpl(fakeSystemPropertiesMap.map()), new EnqueuedTimeImpl("2010-01-01T00:00:00"), new EventOffsetImpl("0")
        );
