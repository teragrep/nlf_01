...
import com.teragrep.akv_01.event.PlainEvent;
...
class SQLSecurityAuditEventsTypeTest {
    ...
    void equalsVerifierTest() {
        EqualsVerifier.forClass(SQLSecurityAuditEventsType.class).verify();
    }

    @Test
    @DisplayName("test msg() return value")
    void testMsgReturnValue() {
        final SQLSecurityAuditEventsType sqlSecurityAuditEventsType = new SQLSecurityAuditEventsType(
                new PlainEvent(
                        new UnparsedEventImpl(
                                ...
                                new EnqueuedTimeImpl(""),
                                new EventOffsetImpl("1")
                        )
                ),
                "realHostName",
                "aer"
        );
        final String msg = Assertions.assertDoesNotThrow(sqlSecurityAuditEventsType::msg);
        final String expectedMsg = "payload";
    ...
    void testIdealCase() {
        final ParsedEvent parsedEvent = testEvent(
                "src/test/resources/sqlsecurityauditevents.json", new EventPartitionContextFake(),
                new EventPropertiesFake(), new EventSystemPropertiesFake(), new EnqueuedTimeImpl("2010-01-01T00:00:00"), new EventOffsetImpl("0")
        );

        final SQLSecurityAuditEventsType type = new SQLSecurityAuditEventsType(parsedEvent, "localhost", "aer");
    ...
    void testWithAllMetadataStubs() {
        final ParsedEvent parsedEvent = testEvent(
                "src/test/resources/sqlsecurityauditevents.json", new EventPartitionContextStub(),
    ...
    void testWithMissingJsonKeys() {
        final ParsedEvent parsedEvent = testEvent(
                "src/test/resources/sqlsecurityauditevents_missing_keys.json", new EventPartitionContextStub(),
                new EventPropertiesStub(), new EventSystemPropertiesStub(), new EnqueuedTimeStub(),
                new EventOffsetStub()
        );
        ...
        Assertions.assertEquals(Facility.AUDIT, actualFacility);
        Assertions.assertEquals("{}", actualMsg);
        Assertions.assertEquals("", actualMsgId);
        Assertions.assertEquals(Severity.NOTICE, actualSeverity);
    ...
    void testSdElementReturnValue() {
        ...
        final ParsedEvent parsedEvent = testEvent(
                "src/test/resources/sqlsecurityauditevents.json", new EventPartitionContextImpl(partitionContextMap), new EventPropertiesImpl(propertiesMap), new EventSystemPropertiesImpl(systemPropertiesMap), new EnqueuedTimeImpl("2010-01-01T00:00:00"), new EventOffsetImpl("0")
        );
