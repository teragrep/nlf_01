...
class PowerPlatformAdminActivityTypeTest {
    ...
    void testIdealCase() {
        final ParsedEvent parsedEvent = testEvent(
                "src/test/resources/powerplatformadminactivity.json", new EventPartitionContextFake(),
                new EventPropertiesFake(), new EventSystemPropertiesFake(), new EnqueuedTimeImpl("2010-01-01T00:00:00"), new EventOffsetImpl("0")
        );

        final PowerPlatformAdminActivityType type = new PowerPlatformAdminActivityType(parsedEvent, "localhost", "aer");
